							...
							<div class="col-xl-12">
									...
									<div class="card-header border-0 pt-6">
										<div class="card-title">
											<h3 class="card-title align-items-start flex-column">
												<span class="card-label fw-bolder fs-3 mb-1">Order List - Supplier</span>
											</h3>
										</div>
                                        <div class="card-toolbar">
											...
											<button class="btn btn-danger btn-sm" type="button" onClick="window.location.href='order-list'" style="margin:0 5px 0 5px;">clear</button>
                                ...
                                {{-- <div class="card mb-6">
                                    <div class="card-header border-0 pt-6">
										<div class="card-title">
											<h3 class="card-title align-items-start flex-column">
												<span class="card-label fw-bolder fs-3 mb-1">Pending From Supplier Side</span>
											</h3>
										</div>
                                    </div>
												...
												<table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_table_users">
													<thead>
														<tr class="fw-bolder fs-6 text-gray-800 px-7">
															...
															<th>Date</th>
															<th>Days</th>
                                                            @if(!empty($permission) && ($permission->full == 1 && (in_array(Auth::user()->user_type, array(1)) || Auth::user()->id ==721 || Auth::user()->id ==496) ) )
                                                                <th>Customer</th>
                                                            @endif
                                                            <th>Supplier Name</th>
													...
													<tbody id="render_string" class="render_string">
														@if(!empty($orders))
                                                                    ...
                                                                    <tr class="{{ $color }}">
                                                                        <td>
                                                                            ...
                                                                            <label class="checkbox justify-content-center">
                                                                                <input class="check_box" data-stone={!! 1 !!}  data-orders_id="<?= $value->orders_id ?>" data-customer_id="<?= $value->customer_id ?>"
                                                                        ...
                                                                        <td>
                                                                            <span class="badge badge-circle badge-primary badge-lg">{{ intval((time() - strtotime($value->orderdetail->created_at))/(60*60*24)) }}</span>
                                                                        </td>
                                                                        @if(!empty($permission) && ($permission->full == 1 && (in_array(Auth::user()->user_type, array(1)) || Auth::user()->id ==721 || Auth::user()->id ==496) ) )
															...
															<tr class="{{ $color }}">
																...
																<td>
                                                                    @if ($value->order_status == 'APPROVED')
                                                                        @if($value->internal_confirmation == "PENDING")
                                                                            <span class="mb-1 text-info fw-bolder">{{ $value->internal_confirmation }}</span><br/>
                                                                            <button class="btn btn-sm btn-icon btn-primary me-1 w-25px h-25px status_internal_confirmation" data-order="{{ $value->orders_id }}"data-status="APPROVED" ><i class="fa fa-check"></i></button>
                                                                        <button class="btn btn-sm btn-icon btn-danger w-25px h-25px status_internal_confirmation" data-order="{{ $value->orders_id }}" data-status="REJECT" ><i class="fa fa-times"></i></button></td>
                                                                        @elseif($value->internal_confirmation == "APPROVED")
                                                                            <span class="mb-1 text-success fw-bolder">Approved</span>
                                                                        @elseif($value->internal_confirmation == "REJECT")
                                                                            <span class="mb-1 text-danger fw-bolder">Rejected</span>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    @endphp
                                                                    <span class="svg-icon svg-icon-2hx {{ $priority_color }}"data-bs-toggle="tooltip" data-bs-original-title="Priority: {{ $priority }}">
                                                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                            <path opacity="0.3" d="M20.9 12.9C20.3 12.9 19.9 12.5 19.9 11.9C19.9 11.3 20.3 10.9 20.9 10.9H21.8C21.3 6.2 17.6 2.4 12.9 2V2.9C12.9 3.5 12.5 3.9 11.9 3.9C11.3 3.9 10.9 3.5 10.9 2.9V2C6.19999 2.5 2.4 6.2 2 10.9H2.89999C3.49999 10.9 3.89999 11.3 3.89999 11.9C3.89999 12.5 3.49999 12.9 2.89999 12.9H2C2.5 17.6 6.19999 21.4 10.9 21.8V20.9C10.9 20.3 11.3 19.9 11.9 19.9C12.5 19.9 12.9 20.3 12.9 20.9V21.8C17.6 21.3 21.4 17.6 21.8 12.9H20.9Z" fill="currentColor"/>
                                                                            <path d="M16.9 10.9H13.6C13.4 10.6 13.2 10.4 12.9 10.2V5.90002C12.9 5.30002 12.5 4.90002 11.9 4.90002C11.3 4.90002 10.9 5.30002 10.9 5.90002V10.2C10.6 10.4 10.4 10.6 10.2 10.9H9.89999C9.29999 10.9 8.89999 11.3 8.89999 11.9C8.89999 12.5 9.29999 12.9 9.89999 12.9H10.2C10.4 13.2 10.6 13.4 10.9 13.6V13.9C10.9 14.5 11.3 14.9 11.9 14.9C12.5 14.9 12.9 14.5 12.9 13.9V13.6C13.2 13.4 13.4 13.2 13.6 12.9H16.9C17.5 12.9 17.9 12.5 17.9 11.9C17.9 11.3 17.5 10.9 16.9 10.9Z" fill="currentColor"/>
                                                                        </svg>
                                                                ...
                                                                <td nowrap>
                                                                    @if($value->supplier_status == "PENDING")
																		<span class="mb-1 text-info fw-bolder">Pending</span><br/>
                                                                        <button class="btn btn-sm btn-icon btn-primary me-1 w-25px h-25px order_approve_supplier" data-order="{{ $value->orders_id }}" data-certino="{{ $value->certificate_no }}" data-status="APPROVED" ><i class="fa fa-check"></i></button>
                                                                        <button class="btn btn-sm btn-icon btn-danger w-25px h-25px order_reject_supplier" data-reject="{{ $value->orders_id }}" data-certino="{{ $value->certificate_no }}" data-status="REJECT" ><i class="fa fa-times"></i></button></td>
																	@elseif($value->supplier_status == "APPROVED")
																		<span class="mb-1 text-success fw-bolder">Approved</span>
																	@elseif($value->supplier_status == "REJECT")
																		<span class="mb-1 text-danger fw-bolder">Rejected</span>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#countryddl').change(function(e){
				...
				location.href = "{{ url('order-list') }}?country="+country;
			});
            $('#holdfilter').change(function(e){
				var hold = $('#holdfilter').val();
				location.href = "{{ url('order-list') }}?hold="+hold;
			});
