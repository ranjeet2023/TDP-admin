                        ...
                        <div class="card card-custom gutter-b">
                            <div class="card-header border-0">
                                <h3 class="card-title align-items-start flex-column">
                                    <span class="card-label fw-bolder text-dark">Page Visits</span>
                                </h3>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                                ...
                                                <tbody id="render_string">
                                                    @if (!empty($records))
														@foreach ($records as $record)
															<tr>
																<td>{{ ($record->users != null) ? $record->users->firstname : '' }}</td>
																<td>@if ($record->user_type == 1)
                                                                        ADMIN
                                                                    @elseif ($record->user_type == 4)
                                                                        Customer Manager
                                                                    @elseif ($record->user_type == 5)
                                                                        Supplier Manager
                                                                    @endif
                                                                    </td>
																<td>{{ $record->path }}</td>
