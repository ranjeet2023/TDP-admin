...
class WhitelabelApiController extends Controller
{
    public function GetConfig(Request $request)
    {
        ...
        if(!empty($token))
        {
            $white_label = WhiteLabel::select('*', DB::raw('(SELECT currency_symbol FROM currency_exchange WHERE currency_exchange.currency_name = white_label.currency_type) as currency_symbol'))->where('token_key',$token)->first();
            if(!empty($white_label))
            {
                ...
                $data['color'] = !empty($white_label->color) ? explode(',',$white_label->color) : ['D','E','F','G'];
                $data['clarity']   = !empty($white_label->clarity) ? explode(',',$white_label->clarity) : ['FL','IF','VVS1', 'VVS2'];
                $data['fancy_color']   = array('yellow', 'pink', 'blue', 'red', 'brown', 'green', 'purple', 'orange','violet', 'grey', 'black', 'champange','other');//explode(',',$white_label->clarity);
                $data['lab']       = !empty($white_label->lab) ? explode(',',$white_label->lab) : ['GIA','IGI'];
                $data['cut']       = !empty($white_label->cut) ? explode(',',$white_label->cut) : ['EX','VG','GD'];
                ...
                $data['currency_type'] = $white_label->currency_name;
                $data['currency_symbol'] = $white_label->currency_symbol;
                $data['colorcode'] = $white_label->theme_color;

                $data['text1'] = '';//Free insured shipping by Tuesday, Feb 14 if you order today.
                $data['text2'] = '';//Free 30 Day Returns, Free Resizing. Free Lifetime Warranty.
                $data['text3'] = 'We inspect & verify authenticity before shipping.';
    ...
    public function whiteSearchDiamond(Request $request)
    {
        if(!empty($sortby))
        {
        ...
        if(!empty($white_label) && !empty($white_label->user_id))
        {
            ...
            if(empty($request->diamond_type))
            {
                $request->diamond_type = 'NATURAL';
            }
            ...
            else
            {
                ...
                elseif(!empty($request->fancy_color))
                {
                    $fancy_color = explode(',',$request->color);
            ...
            {
                $sort_fields = explode(",", $request->sort_field);

                if(in_array('carat', $sort_fields))
                {
                    ...
                    {
                        $result_query->orderBy('carat', 'desc');
                    }
                }

                if(in_array('clarity', $sort_fields))
                {
                    if(strtolower($request->sort_order) == 'asc')
                    {
                        $order = "FIELD(clarity, 'FL','IF','VVS1','VVS2','VS1','VS2','SI1','SI2','SI3','I1','I2')";
                        $result_query->orderByRaw($order);
                    }

                    if(strtolower($request->sort_order) == 'desc')
                    {
                        $order = "FIELD(clarity, 'I2','I1','SI3','SI2','SI1','VS2','VS1','VVS2','VVS1','IF','FL')";
                        $result_query->orderByRaw($order);
                    }
                }

                // if($sort_field == "color")
                if(in_array('color', $sort_fields))
                {
                    if(strtolower($request->sort_order) == 'asc')
                    {
                    ...
                    if(strtolower($request->sort_order) == 'desc')
                    {
                        $result_query->orderBy('color', 'desc');
                    }
                }

                // if($sort_field == "pricepercts")
                if(in_array('pricepercts', $sort_fields))
                {
                    //carat, clarity, color, doller per carat, total :-: asc, desc
                    if(strtolower($request->sort_order) == 'asc')
                    {
                        $result_query->orderBy('rate', 'asc');
                    }

                    if(strtolower($request->sort_order) == 'desc')
                    {
                        $result_query->orderBy('rate', 'desc');
                    }
                }

                // if($sort_field == "totalprice")
                if(in_array('price', $sort_fields))
                {
                    //carat, clarity, color, doller per carat, total :-: asc, desc
                    if(strtolower($request->sort_order) == 'asc')
                    {
                        $result_query->orderBy('net_dollar', 'asc');
                    }

                    if(strtolower($request->sort_order) == 'desc')
                    {
                        $result_query->orderBy('net_dollar', 'desc');
                    }
                }
            }
            else
            ...
            if (!empty($white_label->currency_type) && in_array($white_label->currency_type, array('AUD','USD','EUR'))) {
                $rate = CurrencyExchange::select('currency_rate','currency_symbol')->where('currency_name','=', $white_label->currency_type)->first();
                if(empty($rate)){
                    $currency_rate = 1;
                    $currency_symbol = '$';
                }
            ...
            foreach ($updatedItems as $value) {
                ...
                $d_result['currency_symbol'] = $currency_symbol;
    ...
    public function addDiamond(Request $request) //wish list
    {
    ...
    public function inquiryDiamond(Request $request)
    {
